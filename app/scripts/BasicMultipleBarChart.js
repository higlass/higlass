...
class BasicMultipleBarChart extends BarTrack {
  ...
  renderTile(tile) {
    ...
    for (let i = 0; i < matrix[0].length; i++) { // 15
      ...
      const lineInterval = trackHeight / matrixDimensions[0];
      ...
      for (let j = 0; j < matrix.length; j++) { // 3000
        ...
        const y = (this.position[1] + (lineInterval * i) + lineInterval);
        const height = valueToPixels(matrix[j][i]) - lineInterval;
