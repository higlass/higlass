...
class HorizontalTiled1DPixiTrack extends Tiled1DPixiTrack {
  ...
  drawConstIndicator() {
    ...
    while (
      ...
      this.constIndicator.removeChild(
        this
          .constIndicator
          .children[0]
      );
    ...
    this.options.constIndicators.forEach(
      ({
        ...
        const labelColorHex = colorToHex(
          labelColor
        );

        this.constIndicator.beginFill(
          colorHex,
          opacity
        );

        const y = this.valueScale(
          value
        );
        let xOffset = 0;
        let widthOffset = 0;
