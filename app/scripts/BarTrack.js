...
class BarTrack extends HorizontalLine1DPixiTrack {
  ...
  setColorScale(
    ...
    // rendering.
    this.colorScale = this.colorScale.map(
      rgb =>
        rgb.map(
          channel =>
            channel /
  ...
  renderTile(
    ...
    if (
      ...
      const canvas = gradient(
        ...
        0,
        this
          .dimensions[1] // fromX, fromY, toX, toY
      );
  ...
  drawZeroLine() {
    ...
    const demarcationOpacity = Number.isNaN(
      +this
        .options
        .zeroLineOpacity
    )
  ...
  drawZeroLineSvg(
    ...
    const zeroLine = document.createElement(
      'rect'
    );
  ...
  addSVGInfo(
    ...
    if (
      ...
      tile.svgData.barXValues.push(
        x
      );
      tile.svgData.barYValues.push(
        y
      );
      tile.svgData.barWidths.push(
        width
      );
      tile.svgData.barHeights.push(
        height
      );
      tile.svgData.barColors.push(
        color
      );
    } else {
      tile.svgData = {
      ...
      .forEach(
        tile => {
          ...
          for (
            ...
            const rect = document.createElement(
              'rect'
            );
            rect.setAttribute(
              'fill',
