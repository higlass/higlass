...
class Autocomplete extends React.Component {
  constructor(
    ...
    this.keyDownHandlers = {
      ArrowDown(
        ...
        const itemsLength = this.getFilteredItems()
          .length;
        if (
          !itemsLength
        )
  ...
  maybeScrollItemIntoView() {
    if (
      ...
      const itemNode = this
        .refs[
        `item-${this.state.highlightedIndex}`
      ];
      const menuNode = this
        .refs
        .menu;
  ...
  maybeAutoCompleteText() {
    ...
    const matchedItem =
      ...
      null
        ...
        : items[0];
    const itemValue = this.props.getItemValue(
      matchedItem
    );
  ...
  setMenuPositions() {
    ...
    const computedStyle = global.window.getComputedStyle(
      node
    );
    const marginBottom =
      parseInt(
  ...
  renderMenu() {
    const items = this.getFilteredItems().map(
      (
        item,
        index
      ) => {
      ...
      >
        <input
          ...
          onBlur={this.composeEventHandlers(
            this.handleInputBlur.bind(
              this
            ),
            inputProps.onBlur &&
              inputProps.onBlur.bind(
                this
              )
          )}
          ...
          onClick={this.composeEventHandlers(
            this.handleInputClick.bind(
              this
            ),
            inputProps.onClick &&
              inputProps.onClick.bind(
                this
              )
          )}
          onFocus={this.composeEventHandlers(
            this.handleInputFocus.bind(
              this
            ),
            inputProps.onFocus &&
              inputProps.onFocus.bind(
                this
              )
          )}
...
Autocomplete.defaultProps = {
  ...
  menuStyle: {
    ...
    overflow:
      'auto',
    maxHeight:
      '50%' // TODO: don't cheat, let it flow to the bottom
  },
  autoHighlight: true,
