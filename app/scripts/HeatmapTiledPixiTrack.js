...
class HeatmapTiledPixiTrack extends TiledPixiTrack {
  constructor(
    ...
    this.pMasked.addChild(
      this
        .pColorbarArea
    );

    this.pColorbar = new PIXI.Graphics();
    this.pColorbarArea.addChild(
      this
        .pColorbar
    );
    ...
    this.gMain = this.gBase.append(
      'g'
    );
    this.gColorscaleBrush = this.gMain.append(
      'g'
    );
    ...
    */
    ...
    this.setDataLensSize(
      11
    );
    this.dataLens = new Float32Array(
      this
        .dataLensSize **
        2
    );
    ...
    this.prevOptions = JSON.stringify(
      options
    );
  }
  ...
  getAbsTileDim(
    ...
    if (
      ...
      // triangular matrix
      dim.x = this._refXScale(
        tileY
      );
      dim.y = this._refYScale(
        tileX
      );
    } else {
      dim.x = this._refXScale(
  ...
  newBrushOptions(
    ...
    const endDomain = axisValueScale.invert(
      selection[0]
    );
    const startDomain = axisValueScale.invert(
      selection[1]
    );
    ...
    const startPercent =
      (startDomain -
        axisValueScale.domain()[0]) / // eslint-disable-line operator-linebreak
      (axisValueScale.domain()[1] -
    ...
    newOptions.scaleStartPercent = startPercent.toFixed(
      SCALE_LIMIT_PRECISION
    );
    newOptions.scaleEndPercent = endPercent.toFixed(
      SCALE_LIMIT_PRECISION
    );
  ...
  brushMoved() {
    ...
    const newOptions = this.newBrushOptions(
      event.selection
    );
  ...
  drawColorbar() {
    ...
    if (
      ...
      this.scaleBrush
        .on(
          'start',
          this.brushStart.bind(
            this
          )
        )
        .on(
          'brush',
          this.brushMoved.bind(
            this
          )
        )
        .on(
    ...
    for (
      ...
      const value = this.limitedValueScale(
        axisValueScale.invert(
          i
        )
      );

      const rgbIdx = Math.max(
        0,
        Math.min(
          254,
          Math.floor(
    ...
    this.pAxis.position.y = posScale(
      0
    );
  ...
  exportColorBarSVG() {
    const gColorbarArea = document.createElement(
      'g'
    );
    ...
    const rectColorbarArea = document.createElement(
      'rect'
    );
    gColorbarArea.appendChild(
      rectColorbarArea
    );

    const gColorbar = document.createElement(
      'g'
    );
    gColorbarArea.appendChild(
      gColorbar
    ...
    for (
      ...
      const rectColor = document.createElement(
        'rect'
      );
      gColorbar.appendChild(
        rectColor
      );
    ...
    const gAxisHolder = document.createElement(
      'g'
    );
    gColorbarArea.appendChild(
      gAxisHolder
    ...
    if (
      ...
      gAxis = this.axis.exportAxisRightSVG(
        axisValueScale,
        this
          .colorbarHeight
      );
      ...
      gAxis = this.axis.exportAxisLeftSVG(
        axisValueScale,
        this
          .colorbarHeight
      );
  ...
  getVisibleRectangleData(
    ...
    const leftXBin = Math.floor(
      limitedXDomain[0] /
        tileRes
    );
    const leftYBin = Math.floor(
      limitedYDomain[0] /
        tileRes
    );
    ...
    this.visibleAndFetchedTiles().forEach(
      tile => {
        ...
        const tileXStartBin = Math.floor(
          tileX /
            tileRes
        );
        const tileXEndBin = Math.floor(
          (tileX +
            tileWidth) /
            tileRes
        );
        const tileYStartBin = Math.floor(
          tileY /
            tileRes
        );
        const tileYEndBin = Math.floor(
          (tileY +
            tileHeight) /
            tileRes
        );
        ...
        const tileXOffset = Math.max(
          tileXStartBin -
            leftXBin,
          0
        );
        const tileYOffset = Math.max(
          tileYStartBin -
            leftYBin,
          0
        );
        const tileSliceWidth =
          tileSliceXEnd -
          tileSliceXStart;
        const tileSliceHeight =
          tileSliceYEnd -
          tileSliceYStart;
    ...
    tileProxy.tileDataToPixData(
      ...
      pixData => {
        ...
        if (
          ...
          const canvas = this.tileDataToCanvas(
            pixData.pixData
          );
  ...
  getMouseOverHtml(
    ...
    const dataX = this._xScale.invert(
      trackX
    );
    const dataY = this._yScale.invert(
      trackY
    );
    ...
    let data = null;
    try {
      data = this.getVisibleRectangleData(
        trackX,
        trackY,
        1,
        1
    ...
    if (
      ...
      if (
        ...
        return `${positionText}<b>Value:</b> 1e${format(
          '.3f'
        )(
          Math.log(
            data
    ...
    return `${positionText}<b>Value:</b> ${format(
      '.3f'
    )(
      data
    )}`;
