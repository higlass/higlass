...
export function fetchMultiRequestTiles(
  req,
  pubSub
) {
  const requests =
  ...
  for (const server of servers) {
    ...
    for (
      ...
      const renderParams = theseTileIds
        .map(
          x =>
            `d=${x}`
        )
 ...
 */
export const calculateZoomLevelFromResolutions = (
  resolutions,
  scale
) => {
  ...
  const binsPerPixel = binsDisplayed.map(
    b =>
      b /
      trackWidth
  );
  ...
  // pixel per bin
  const displayableBinsPerPixel = binsPerPixel.filter(
    b =>
      b <
      1
  );
  ...
  if (
    displayableBinsPerPixel.length ===
    0
  )
    return 0;

  return binsPerPixel.indexOf(
    displayableBinsPerPixel[
      displayableBinsPerPixel.length -
        1
    ]
  );
...
export const calculateTilesFromResolution = (
  ...
  if (
    ...
    maxX =
      Number.MAX_VALUE; // eslint-disable-line no-param-reassign
  }
...
export const trackInfo = (
  ...
  const url = `${tts(
    server
  )}/tileset_info/?d=${tilesetUid}&s=${sessionId}`;
  pubSub.publish(
    'requestSent',
    url
  );
    ...
    (
      ...
      if (
        ...
        if (
          ...
          errorCb(
            `Error retrieving tilesetInfo from: ${server}`
          );
        } else {
          console.warn(
            'Error retrieving: ',
...
function fetchEither(
  ...
  if (
    ...
    throw new Error(
      `fetch either "text" or "json", not "${textOrJson}"`
    );
  }
  const headers = {
    'Content-Type': mime
