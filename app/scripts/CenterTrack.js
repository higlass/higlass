...
class CenterTrack extends React.Component {
  ...
  shouldComponentUpdate(
    ...
    if (
      this
        .rangeSelectionTriggeredXY
    ) {
      this.rangeSelectionTriggeredXY = false;
      if (
        this
          .rangeSelectionTriggeredXYEnd
      ) {
        const dim1 =
          nextProps
            .rangeSelection[0] ||
  ...
  addBrush1d() {
    ...
    resetD3BrushStyle(
      this
        .brushElX,
      stylesTrack[
        'track-range-selection-group-brush-selection'
      ]
    );
    resetD3BrushStyle(
      this
  ...
  brushedX() {
    ...
    const rangeSelectionMoved = this
      .rangeSelectionMoved;
  ...
  brushedXEnded() {
    const rangeSelectionMovedEnd = this
  ...
  brushedYEnded() {
    ...
    this.props.onRangeSelectionYEnd(
      event.selection
    );
  }
  ...
  render() {
    ...
    const menuClash = this.props.tracks.some(
      track => {
        if (
          track.contents
        ) {
          // if this is a combined track, iterate over children
          return track.contents.some(
            subTrack => {
              if (
                subTrack.type ===
                'heatmap'
    ...
    const rangeSelectorGroup1dClass = !this
      .props
      .is1dRangeSelection
      ? 'stylesTrack.track-range-selection-group-inactive'
      : '';

    const rangeSelectorGroup2dClass = this
      .props
      .is1dRangeSelection
      ? 'stylesTrack.track-range-selection-group-inactive'
      : '';
    ...
    return (
      <div
        ...
        onMouseLeave={this.mouseLeaveHandler.bind(
          this
        )}
        style={{
          height: this
      ...
      >
        {isBrushable && (
          ...
          >
            <g
              ref={el => {
                this.brushElX = select(
                  el
                );
              }}
              styleName={
                rangeSelectorGroup1dClass
              }
            />
            <g
              ref={el => {
                this.brushElY = select(
                  el
                );
              }}
              styleName={
            ...
            <g
              ref={el => {
                this.brushElXY = select(
                  el
                );
              }}
              styleName={
                rangeSelectorGroup2dClass
              }
            />
          </svg>
        )}
...
CenterTrack.propTypes = {
  ...
  onRangeSelectionXY:
    PropTypes
      .func
      .isRequired,
  onRangeSelectionXYEnd:
    PropTypes
      .func
      .isRequired,
  onRangeSelectionReset:
    PropTypes
      .func
      .isRequired,
