...
function _uint16ArrayToFloat32Array(
  uint16array
) {
  ...
  const newBytes = new Float32Array(
    bytes.buffer
  );
 ...
 */
export function tileResponseToData(
  data,
  server,
  theseTileIds
) {
  ...
  for (const thisId of theseTileIds) {
    ...
    if (
      ...
      const arrayBuffer = _base64ToArrayBuffer(
        data[
          key
        ]
          .dense
      ...
      if (
        ...
        const newDense = _uint16ArrayToFloat32Array(
          uint16Array
        );
        a = newDense;
      } else {
