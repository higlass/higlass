...
class HorizontalGeneAnnotationsTrack extends HorizontalTiled1DPixiTrack {
  ...
  constructor(
    ...
    this.geneStrandSpacing =
      +this
        ...
        .geneStrandSpacing ||
      GENE_STRAND_SPACING;
    this.geneStrandHSpacing =
      this
        .geneStrandSpacing /
      2;
    this.geneRectHHeight =
      this
        .geneRectHeight /
      2;
  }
  ...
  initTile(
    ...
    tile.tileData = tile.tileData.slice(
      0,
      MAX_TILE_ENTRIES
    );
    ...
    tile.tileData.forEach(
      (
        ...
        tile.texts[
          geneId
        ] = text; // index by geneName

        tile.textGraphics.addChild(
          text
        );
      }
    );
  ...
  destroyTile(
    ...
    const zoomLevel = +tile.tileId.split(
      '.'
    )[0];
    ...
    tile.tileData
      ...
      .forEach(
        (
          ...
          const xStartPos = this._xScale(
            txStart
          );
          const xEndPos = this._xScale(
            txEnd
          );
          ...
          if (
            ...
            const textWidth = text.getBounds()
              .width;
            const textHeight = text.getBounds()
              .height;
  ...
  drawExons(
    ...
    const exonOffsetStarts = exonStarts
      .split(
        ','
      )
      .map(
    ...
    Object.values(
      ...
      .filter(
        tile => {
          ...
          const tileK =
            (tile.drawnAtScale.domain()[1] -
              tile.drawnAtScale.domain()[0]) /
    ...
    Object.values(
      ...
      .forEach(
        tile => {
          ...
          // move the texts
          const parentInFetched = this.parentInFetched(
            tile
          );
  ...
  exportSVG() {
    ...
    this.visibleAndFetchedTiles()
      ...
      .forEach(
        tile => {
          const gTile = document.createElement(
            'g'
          );
          gTile.setAttribute(
            'transform',
