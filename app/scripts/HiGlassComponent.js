...
class HiGlassComponent extends React.Component {
  constructor(
    ...
    const views = this.loadIfRemoteViewConfig(
      this
        .props
        .viewConfig
    );
    ...
    // Bound functions
    this.appClickHandlerBound = this.appClickHandler.bind(
      this
    );
    this.keyDownHandlerBound = this.keyDownHandler.bind(
      this
    );
    this.keyUpHandlerBound = this.keyUpHandler.bind(
      this
    );
    ...
    this.dispatchEventBound = this.dispatchEvent.bind(
      this
    );
    this.animateOnMouseMoveHandlerBound = this.animateOnMouseMoveHandler.bind(
      this
    );
    this.zoomStartHandlerBound = this.zoomStartHandler.bind(
      this
    );
    this.zoomEndHandlerBound = this.zoomEndHandler.bind(
      this
    );
    this.zoomHandlerBound = this.zoomHandler.bind(
      this
    );
    this.trackDroppedHandlerBound = this.trackDroppedHandler.bind(
      this
    );
    this.animateBound = this.animate.bind(
      this
    );
    this.animateOnGlobalEventBound = this.animateOnGlobalEvent.bind(
      this
    );
    this.requestReceivedHandlerBound = this.requestReceivedHandler.bind(
      this
    );
    this.wheelHandlerBound = this.wheelHandler.bind(
      this
    );
    this.mouseMoveHandlerBound = this.mouseMoveHandler.bind(
      this
    );
    this.onMouseLeaveHandlerBound = this.onMouseLeaveHandler.bind(
      this
    );
    this.onBlurHandlerBound = this.onBlurHandler.bind(
      this
    );
    this.openModalBound = this.openModal.bind(
      this
    );
    this.closeModalBound = this.closeModal.bind(
      this
    );
    this.handleEditViewConfigBound = this.handleEditViewConfig.bind(
      this
    );
    ...
    this.setBroadcastMousePositionGlobally(
      this
        .props
        .options
        .broadcastMousePositionGlobally ||
        this
          .props
          .options
          .globalMousePosition
  ...
  UNSAFE_componentWillMount() {
    ...
    this.pubSubs.push(
      ...
      this.pubSub.subscribe(
        ...
        this
          .animateOnMouseMoveHandlerBound
      ),
      this.pubSub.subscribe(
        'trackDropped',
        this
          .trackDroppedHandlerBound
      ),
      this.pubSub.subscribe(
        'app.zoomStart',
        this
          .zoomStartHandlerBound
      ),
      this.pubSub.subscribe(
        'app.zoomEnd',
        this
  ...
  waitForDOMAttachment(
    ...
    const thisElement = ReactDOM.findDOMNode(
      this
    );
    ...
    Object.values(
      ...
      view => {
        if (
          ...
          view.layout = this.generateViewLayout(
            view
          );
        } else {
          view.layout.i =
  ...
  getTrackObject(
    ...
    return this.tiledPlots[
      viewUid
    ].trackRenderer.getTrackObject(
      trackUid
    );
  }
  ...
  UNSAFE_componentWillReceiveProps(
    ...
    const viewsByUid = this.loadIfRemoteViewConfig(
      newProps.viewConfig
    );
    ...
    if (
      ...
      for (const viewId of this.iterateOverViews()) {
        const trackRenderer = this.getTrackRenderer(
          viewId
        );
        const trackDefinitions = JSON.parse(
          trackRenderer.prevTrackDefinitions
        );
  ...
  setTheme(
    ...
    if (
      ...
      switch (
        ...
        default:
          console.warn(
            `Unknown theme "${newTheme}". Using light theme.`
          );
          this.theme = THEME_LIGHT;
          break;
      }
  ...
  addDefaultTrackOptions(
    ...
    if (
      typeof track.options ===
      'undefined'
    ) {
      track.options = {};
    }

    const trackOptions = track.options
      ? track.options
      : {};
  ...
  handleLockLocation(
    ...
    this.setState(
      {
        chooseViewHandler: uid2 =>
          this.handleLocationLockChosen(
            uid,
            uid2
          ),
        mouseOverOverlayUid: uid
  ...
  iterateOverViews() {
    ...
    for (const viewId in Object.keys(
      this
        .state
        .views
    )) {
  ...
  syncValueScales(
    ...
    const uid = this.combineViewAndTrackUid(
      viewUid,
      trackUid
    );
    ...
    if (
      ...
      const lockGroupValues = dictValues(
        this
          .valueScaleLocks[
          uid
        ]
   ...
   */
  addDraggingChangedListener(
    viewUid,
    listenerUid,
    eventHandler
  ) {
    if (
      !this.draggingChangedListeners.hasOwnProperty(
        viewUid
      )
    ) {
      this.draggingChangedListeners[
  ...
  createSVG() {
    ...
    for (const tiledPlot of dictValues(
      ...
      if (
        ...
        continue; // probably opened and closed
  ...
  createPNGBlobPromise() {
    return new Promise(
      resolve => {
        ...
        img.onload = () => {
          const targetCanvas = document.createElement(
            'canvas'
          );
  ...
  handleExportPNG() {
    this.createPNGBlobPromise().then(
      blob => {
        download(
          'export.png',
          blob
  ...
  handleScalesChanged(
    ...
    if (
      ...
      const lockGroup = this
        .zoomLocks[
        uid
      ];
      const lockGroupItems = dictItems(
        lockGroup
      );
      ...
      for (
        ...
        if (
          this.scalesChangedListeners.hasOwnProperty(
            key
          )
        ) {
          dictValues(
  ...
  handleUnlock(
    ...
    const lockGroupKeys = dictKeys(
      lockGroup
    );

    if (
      lockGroupKeys.length ===
      2
    ) {
  ...
  viewScalesLockData(
    ...
    if (
      ...
      console.warn(
        "View scale lock doesn't correspond to existing uid: ",
        uid
      );
      return null;
    }
  ...
  addLock(
    ...
    const allMembers = group1Members.concat(
      group2Members
    );

    const groupDict = dictFromTuples(
      allMembers
    );
  ...
  handleViewportProjected(
    ...
    if (
      ...
      alert(
        'A view can not show its own viewport.'
      );
    } else {
      const hostTrack = getTrackByUid(
        this
          .state
          .views[
          toView
   ...
   */
  handleLocationYanked(
    uid1,
    uid2
  ) {
    // where we're taking the zoom from
    const sourceXScale = this
      .xScales[
      uid2
    ];
    const sourceYScale = this
      .yScales[
      uid2
    ];

    const targetXScale = this
      .xScales[
      uid1
    ];
    const targetYScale = this
      .yScales[
      uid1
    ];
  ...
  updateRowHeight() {
    ...
    const MARGIN_HEIGHT = this.isEditable()
      ? 10
      : 0;
    ...
    const prospectiveRowHeight =
      availableHeight /
      maxHeight; // maxHeight is the number of
    // rows necessary to display this view

    const chosenRowHeight = Math.floor(
      prospectiveRowHeight
    );
  ...
  refreshView(
    timeout = SHORT_DRAG_TIMEOUT
  ) {
    this.clearDragTimeout();

    this.notifyDragChangedListeners(
      true
    );
  ...
  calculateViewDimensions(
    ...
    if (
      ...
      for (
        ...
        const thisHeight = track.height
          ? track.height
          : defaultCenterHeight;
        currWidth += track.width
          ? track.width
          : defaultVerticalWidth;
  ...
  handleCloseView(
    ...
    this.setState(
      prevState => {
        ...
        const viewsByUid = this.removeInvalidTracks(
          prevState.views
        );
        return {
          views: viewsByUid
  ...
  handleTrackAdded(
    ...
    if (
      ...
      this.zoomToDataExtentOnInit.add(
        viewId
      );
    }
   ...
   */
  compatibilityfyStackedHeatmaps(
    newTrack,
    combinedTrack
  ) {
    let otherHeatmap;

    const hasHeatmaps = combinedTrack.contents.some(
      track => {
        otherHeatmap = track;
        return (
          track.type ===
    ...
    if (
      ...
      newTrack.options.backgroundColor =
        'transparent';
      newTrack.options.showTooltip =
  ...
  storeTrackSizes(
    ...
    visitPositionedTracks(
      this
        ...
        .tracks,
      track => {
        const trackObj = this.tiledPlots[
          viewId
        ].trackRenderer.getTrackObject(
          track.uid
        );
  ...
  handleCloseTrack(
    ...
    for (const trackType in tracks) {
      ...
      const newTracks = theseTracks.filter(
        d =>
          d.uid !==
          uid
      );
      ...
      if (
        ...
        const combinedTracks = newTracks.filter(
          x =>
            x.type ===
            'combined'
        );

        combinedTracks.forEach(
          ct => {
            ct.contents = ct.contents.filter(
              x =>
                x.uid !==
                uid
            );
  ...
  handleValueScaleLocked(
    ...
    const fromUid = this.combineViewAndTrackUid(
      fromViewUid,
      fromTrackUid
    );
    const toUid = this.combineViewAndTrackUid(
      toViewUid,
      toTrackUid
    );
  ...
  addCallbacks(
    ...
    if (
      ...
      track.removeViewportChanged = trackId =>
        this.removeScalesChangedListener(
          fromView,
          trackId
        );
      ...
      for (const viewUid of dictKeys(
        ...
        this.locationLocks[
          ...
          viewConfig.locationLocks.locksDict[
            viewConfig.locationLocks.locksByViewUid[
              viewUid
            ]
          ];
      }
      ...
      for (const viewUid of dictKeys(
        ...
        this.zoomLocks[
          ...
          viewConfig.zoomLocks.locksDict[
            viewConfig.zoomLocks.locksByViewUid[
              viewUid
            ]
          ];
      }
  ...
  deserializeValueScaleLocks(
    viewConfig
  ) {
    this.valueScaleLocks = {};

    if (
      viewConfig.valueScaleLocks
    ) {
      for (const viewUid of dictKeys(
        viewConfig
          .valueScaleLocks
  ...
  getViewsAsJson() {
    ...
    newJson.views = Object.values(
      this
        .state
        .views
    ).map(
      k => {
        const newView = JSON.parse(
          JSON.stringify(
            k
          )
        );
        ...
        newView.uid =
          k.uid;
        newView.initialXDomain = this.xScales[
          k
            .uid
        ].domain();
        newView.initialYDomain = this.yScales[
          k
            .uid
        ].domain();
    ...
    newJson.zoomLocks = this.serializeLocks(
      this
        .zoomLocks
    );
    newJson.locationLocks = this.serializeLocks(
      this
        .locationLocks
    );
    newJson.valueScaleLocks = this.serializeLocks(
      this
        .valueScaleLocks
    );
    ...
    if (
      ...
      req
        ...
        .catch(
          e =>
            console.error(
              'Exporting view config as link failed:',
              e
            )
        );
    }
  ...
  handleDataDomainChanged(
    ...
    views[
      viewUid
    ].initialXDomain = newXDomain;
    views[
      viewUid
    ].initialYDomain = newYDomain;
  ...
  handleSelectedAssemblyChanged(
    ...
    views[
      viewUid
    ].genomePositionSearchBox.chromInfoId = newAssembly;
    views[
      viewUid
    ].genomePositionSearchBox.autocompleteId = newAutocompleteId;
    views[
      viewUid
    ].genomePositionSearchBox.autocompleteServer = newServer;
  }
  ...
  createGenomePostionSearchBoxEntry(
    ...
    if (
      !newGpsb.autocompleteServer
    ) {
      newGpsb.autocompleteServer =
        defaultGpsb.autocompleteServer;
    }
    ...
    if (
      !newGpsb.chromInfoServer
    ) {
      newGpsb.chromInfoServer =
        defaultGpsb.chromInfoServer;
    }
  ...
  handleTogglePositionSearchBox(
    ...
    const sortedAssemblyCounts = dictItems(
      assemblyCounts
    ).sort(
      (
        a,
    ...
    let selectedAssembly =
      'hg19'; // always the default if nothing is otherwise selected

    if (
      sortedAssemblyCounts.length
    ) {
      selectedAssembly =
        sortedAssemblyCounts[0][0];
    }
    ...
    view.genomePositionSearchBox.visible = !view
  ...
  adjustNewTrackOptions(
    ...
    if (
      ...
      if (
        ...
        newOptions.labelPosition =
          'topRight';
        newOptions.colorbarPosition =
          'topRight';
      }
      if (
        newOptions.extent ===
  ...
  adjustOtherTrackOptions(
    ...
    if (
      ...
      if (
        ...
        allTracks.center[0].contents.some(
          otherTrack => {
            if (
              otherTrack.type ===
                'heatmap' &&
  ...
  processViewConfig(
    ...
    if (
      ...
      console.warn(
        'No views provided in viewConfig'
      );
      views = [
        {
          editable: true,
    ...
    views.forEach(
      v => {
        ...
        if (
          this.zoomToDataExtentOnInit.has(
            v.uid
          )
        ) {
          this.zoomToDataExtentOnInit.delete(
            v.uid
          );
        }
        ...
        if (
          ...
          console.warn(
            'No initialXDomain provided in the view config.'
          );
          v.initialXDomain = [
            0,
            100
          ...
          v.initialXDomain[0] = +v
            .initialXDomain[0];
          v.initialXDomain[1] = +v
            .initialXDomain[1];
        }
        ...
        visitPositionedTracks(
          ...
          track => {
            ...
            if (
              ...
              // add default options to combined tracks
              for (const ct of track.contents)
                this.addDefaultTrackOptions(
                  ct
                );
            }
        ...
        if (
          ...
          v.layout = this.generateViewLayout(
            v
          );
        }
      }
    ...
    this.deserializeZoomLocks(
      viewConfig
    );
    this.deserializeLocationLocks(
      viewConfig
    );
    this.deserializeValueScaleLocks(
      viewConfig
    );

    viewsByUid = this.removeInvalidTracks(
      viewsByUid
    );
  ...
  offViewChange(
    ...
    this.viewChangeListener.splice(
      listenerId,
      1
    );
  }
  ...
  zoomTo(
    ...
    if (
      ...
      throw Error(
        `Invalid viewUid. Current uuids: ${Object.keys(
          this
            .setCenters
        ).join(
          ','
        ...
        Object.keys(
          ...
          .map(
            listenerId =>
              parseInt(
                listenerId.slice(
                  LOCATION_LISTENER_PREFIX.length +
                    1
                ),
                10
              )
  ...
  getTiledPlotAtPosition(
    ...
    for (
      ...
      const area = this.tiledAreasDivs[
        views[
          i
        ]
          .uid
  ...
  getMinMaxValue(
    ...
    if (
      !track.minVisibleValue ||
      !track.maxVisibleValue
    ) {
      console.warn(
    ...
    if (
      ignoreOffScreenValues &&
      track.getAggregatedVisibleValue
    ) {
      return [
        track.getAggregatedVisibleValue(
          'min'
    ...
    */
    ...
    mouseOverDiv
      .style(
        'left',
        `${
          normalizedMousePos[0]
        }px`
      )
      .style(
        'top',
        `${
          normalizedMousePos[1]
        }px`
      );
  ...
  setTrackValueScaleLimits(
    ...
    if (
      ...
      console.warn(
        `Could't find track: ${trackId}`
      );
      return;
    }

    if (
      track.setFixedValueScaleMin &&
      track.setFixedValueScaleMax
    ) {
      track.setFixedValueScaleMin(
        minValue
      );
      ...
      console.warn(
        "Track doesn't support fixed value scales."
      );
    }
  }
  ...
  render() {
    this.tiledAreasDivs = {};
    this.tiledAreas = (
      <div styleName="styles.tiled-area" />
    );
    ...
    if (
      ...
      this.tiledAreas = dictValues(
        this
          .state
          .views
      ).map(
        view => {
          const zoomFixed = this.isZoomFixed(
            view
          );
          ...
          const addTrackPositionMenuPosition =
            ...
            this
              .state
              .addTrackPositionMenuUid
              ? this
                  .state
                  .addTrackPositionMenuPosition
              : null;
          ...
          const tiledPlot = (
            <TiledPlot
              ...
              onDataDomainChanged={(
                ...
                this.handleDataDomainChanged(
                  view.uid,
                  xDomain,
                  yDomain
                )
              }
              onLockValueScale={uid =>
                this.handleLockValueScale(
                  view.uid,
                  uid
                )
              }
              onMouseMoveZoom={this.mouseMoveZoomHandler.bind(
                this
              )}
              ...
              onNoTrackAdded={this.handleNoTrackAdded.bind(
                this
              )}
              onRangeSelection={this.rangeSelectionHandler.bind(
                this
              )}
              onScalesChanged={(
                x,
              ...
              onTrackPositionChosen={this.handleTrackPositionChosen.bind(
                this
              )}
              onTracksAdded={(
                newTracks,
              ...
              registerDraggingChangedListener={listener =>
                this.addDraggingChangedListener(
                  view.uid,
                  view.uid,
                  listener
                )
              ...
              removeDraggingChangedListener={listener =>
                this.removeDraggingChangedListener(
                  view.uid,
                  view.uid,
                  listener
                )
          ...
          const looseTracks = positionedTracksToAllTracks(
            view.tracks
          );
          ...
          const getGenomePositionSearchBox = (
            isFocused,
            onFocus
          ) => {
            return (
              <GenomePositionSearchBox
                ...
                onSelectedAssemblyChanged={(
                  ...
                  this.handleSelectedAssemblyChanged(
                    view.uid,
                    x,
                    y,
                    server
          ...
          const multiTrackHeader =
            ...
            !this
              ...
              <ViewHeader
                ...
                onEditViewConfig={
                  this
                    .handleEditViewConfigBound
                }
                onExportPNG={this.handleExportPNG.bind(
                  this
                )}
                onExportSVG={this.handleExportSVG.bind(
                  this
                )}
                onExportViewsAsJSON={this.handleExportViewAsJSON.bind(
                  this
                )}
                onExportViewsAsLink={this.handleExportViewsAsLink.bind(
                  this
                )}
                onLockLocation={uid =>
                  this.handleYankFunction(
                    uid,
                    this.handleLocationLockChosen.bind(
                      this
                    )
                  )
                }
                onLockZoom={uid =>
                  this.handleYankFunction(
                    uid,
                    this.handleZoomLockChosen.bind(
                      this
                    )
                  )
                }
                ...
                onProjectViewport={this.handleProjectViewport.bind(
                  this
                )}
                onTakeAndLockZoomAndLocation={uid => {
                  this.handleYankFunction(
                    ...
                    (
                      ...
                      this.handleZoomYanked(
                        a,
                        b
                      );
                      this.handleLocationYanked(
                        a,
                        b
                      );
                      this.handleZoomLockChosen(
                ...
                onTrackPositionChosen={position =>
                  this.handleTrackPositionChosen(
                    view.uid,
                    position
                  )
                }
                ...
                onYankLocation={uid =>
                  this.handleYankFunction(
                    uid,
                    this.handleLocationYanked.bind(
                      this
                    )
                  )
                }
    ...
    const gridLayout = (
      <ReactGridLayout
        ...
        onBreakpointChange={this.onBreakpointChange.bind(
          this
        )}
        onDragStart={this.handleDragStart.bind(
          this
        )}
        onDragStop={this.handleDragStop.bind(
          this
        )}
        onLayoutChange={this.handleLayoutChange.bind(
          this
        )}
        onResize={this.resizeHandler.bind(
          this
        )}
        rowHeight={
          this
    ...
    return (
      <div
        ...
        onMouseLeave={
          this
            .onMouseLeaveHandlerBound
        }
        onMouseMove={
          this
            .mouseMoveHandlerBound
        }
        styleName={
          styleNames
        }
