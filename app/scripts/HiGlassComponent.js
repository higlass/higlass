...
class HiGlassComponent extends React.Component {
  ...
  syncValueScales(viewUid, trackUid) {
    ...
    if (this.valueScaleLocks[uid]) {
      ...
      const minValues = lockedTracks
        ...
        .map(track =>
          lockGroup.ignoreOffScreenValues ? track.minRawValue() : track.minVisibleValue(true)
        );
      ...
      const maxValues = lockedTracks
        ...
        .filter(track => track.minRawValue && track.maxRawValue)
        .map(track =>
          lockGroup.ignoreOffScreenValues ? track.maxRawValue() : track.maxVisibleValue(true)
        );
  ...
  handleExportSVG() {
    download('export.svg', new Blob([this.createSVGString()], { type: 'image/svg+xml' }));
  }
