...
class BedLikeTrack extends HorizontalTiled1DPixiTrack {
  ...
  initTile(
    ...
    if (
      ...
      if (
        ...
        const segments = tile.tileData.map(
          x => {
  ...
  draw() {
    ...
    for (const fetchedTileId in this
      .fetchedTiles) {
      const tile = this
        .fetchedTiles[
        fetchedTileId
  ...
  getMouseOverHtml(
    ...
    if (
      ...
      const visibleRects = Object.values(
        this
          .drawnRects[
          zoomLevel
        ]
      ...
      for (
        ...
        if (
          ...
          const parts = visibleRects[
            i
          ][1].value.fields.slice(
            3
          );
