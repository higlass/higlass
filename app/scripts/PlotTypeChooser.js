...
// Configs
import { TRACKS_INFO, DATATYPE_TO_TRACK_TYPE, AVAILABLE_TRACK_TYPES } from './configs';
...
class PlotTypeChooser extends React.Component {
  ...
  render() {
    ...
    if (this.AVAILABLE_TRACK_TYPES) {
      AVAILABLE_TRACK_TYPES_LIST = this.AVAILABLE_TRACK_TYPES.sort((a, b) => a.type < b.type).map(
        x => {
          const thumbnail = trackTypeToInfo[x.type].thumbnail;
          const plotTypeClass =
            this.state.selectedPlotType.type === x.type ? 'plot-type-selected' : 'unselected';
    ...
    return (
      <div>
        {AVAILABLE_TRACK_TYPES_LIST.length > 0 && (
          <div className="plot-type-container">{AVAILABLE_TRACK_TYPES_LIST}</div>
        )}
      </div>
    );
  }
