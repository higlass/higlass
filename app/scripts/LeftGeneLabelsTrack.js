...
export function LeftGeneLabelsTrack() {
    let width = 200;
    let height = 15;
            ...
            function redrawTile() {
                ...
                drawTile = function(graphics, tile) {
                    ...
                    for (let i = 0; i < tileData.length; i++) {
                        ...
                        let yEndPos = zoomedYScale(+tileData[i].txEnd + genomeOffset);

                        let yPos = (yEndPos + yStartPos) / 2;

                        //let yPos = -(d.height - yScale(tileData[i]));
                        let height = xScale(Math.log(+tileData[i].count+1))
                        let width = xScale(Math.log(+tileData[i].count+1));
                        let xPos = (d.width - width) / 2 + 5 ; //-(d.height - yScale(tileData[i]));
                ...
                for (let i = 0; i < allTiles.length; i++) {
                    ...
                    if (!(allTiles[i].tileId in d.tileGraphics)) {
                        ...
                        // so we need to create one
                         ...
                         drawTile(newGraphics, allTiles[i], zoomedYScale);
            ...
            function zoomChanged(translate, scale) {
                ...
                if (drawTile != null) {
                    for (let i = 0; i < allTiles.length; i++) {
                        ...
                        drawTile(tileGraphics, allTiles[i], zoomedYScale);
                    }
                }
            }
