...
export default class StackedBarTrack extends BarTrack {
  initTile(tile) {
  ...
  renderTile(tile) {
    ...
    // number of different nucleotides in each bar
    const shapeX = tile.tileData.shape[0];
    // number of bars
    ...
    if (
      flattenedArray.filter(a => a < 0).length > 0 &&
      this.options.valueScaling === 'linear'
    ) {
      console.warn(
        'Negative values present in data. Defaulting to exponential scale.'
      );
      this.options.valueScaling = 'exponential';
      // todo does anything meaningful even happen here
    }

    // un-flatten data into matrix of:
    //  tile.tileData.shape[0] x tile.tileData.shape[1]
    // first array in the matrix will be
    //  [flattenedArray[0], flattenedArray[256], flattenedArray[512], etc.]
    // because of how flattenedArray comes back from the server.
    const matrix = [];
    for (let i = 0; i < shapeX; i++) {  // 6
      for (let j = 0; j < shapeY; j++) {  // 256
        const singleBar = matrix[j] === undefined ? [] : matrix[j];
    ...
    if (this.options.scaledHeight === true) {
      ...
      for (let i = 0; i < matrix.length; i++) {
        ...
        const localPositiveMax = temp
          .filter(a => a >= 0)
          .reduce((a, b) => a + b, 0);
        positiveMax = localPositiveMax > positiveMax
          ? localPositiveMax
          : positiveMax;
        const localNegativeMax = Math.abs(
          temp.filter(a => a < 0).reduce((a, b) => a + b, 0)
        );
        negativeMax = localNegativeMax > negativeMax
          ? localNegativeMax
          : negativeMax;
      }
      this.drawVerticalBars(
        graphics, matrix, tileX, tileWidth, positiveMax, negativeMax
      );
    } else {
   ...
   * @param positiveMax the height of the tallest bar in the positive part of
   *   the graph
   * @param negativeMax the height of the tallest bar in the negative part of
   *   the graph
   */
  drawVerticalBars(
    graphics, matrix, tileX, tileWidth, positiveMax, negativeMax
    ...
    for (let j = 0; j < matrix.length; j++) { // jth vertical bar in the graph
      const x = this._xScale(
        tileX + (j * tileWidth / this.tilesetInfo.tile_size)
      );
      const width = this._xScale(
        tileX + (tileWidth / this.tilesetInfo.tile_size)
      ) - this._xScale(tileX);

      // sorted from largest to smallest
      // we're grouping values along with their index in the column
      const positiveValsSorted = matrix[j]
        .map((_x, i) => [_x, i])
        .filter(a => a[0] >= 0)
        .sort((a, b) => a[0] - b[0]);
      ...
      for (let i = 0; i < positiveValsSorted.length; i++) {
        ...
        const y = (
          (trackHeight - negativeTrackHeight)
          - (prevStackedBarHeight + height)
        );
    ...
    for (let j = 0; j < matrix.length; j++) { // jth vertical bar in the graph
      ...
      // sorted from largest to smallest
      // we're grouping values along with their index in the column
      const negativeValsSorted = matrix[j]
        .map((_x, i) => [_x, i])
        .filter(a => a[0] < 0)
        .sort((a, b) => a[0] - b[0]);
  ...
  drawNormalizedBars(graphics, matrix, tileX, tileWidth) {
    ...
    for (let j = 0; j < matrix.length; j++) { // jth vertical bar in the graph
      ...
      const sorted = matrix[j].sort((a, b) => b - a);
