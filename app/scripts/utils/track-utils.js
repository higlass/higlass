 ...
 *                         of the view.
 */
const calculateZoomLevelFromResolutions = (
  resolutions,
  scale
) => {
...
const drawAxis = (
  ...
  if (
    ...
    track.axis = new AxisPixi(
      track
    );
    track.pBase.addChild(
      track
        .axis
        .pAxis
    );
  ...
  if (
    ...
    track.axis.pAxis.position.x =
      track
        .position[0] +
      margin;
    track.axis.pAxis.position.y =
      track.position[1];

    track.axis.drawAxisRight(
      valueScale,
      track
        .dimensions[1]
    );
...
const movedY = (
  ...
  Object.values(
    ...
    tile => {
      tile.graphics.position.y =
        track.valueScaleTransform.y;
    }
  );
