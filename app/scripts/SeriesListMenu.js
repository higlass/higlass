...
export default class SeriesListMenu extends ContextMenuContainer {
  getConfigureSeriesMenu(
    ...
    for (const optionType of TRACKS_INFO_BY_TYPE[
      track
        .type
    ]
      .availableOptions) {
      if (
        ...
        if (
          ...
          const generatedOptions = OPTIONS_INFO[
            optionType
          ].generateOptions(
            track
          );
            ...
            const optionSelectorSettings = {
              ...
              handler: () => {
                track.options[
                  ...
                  generatedOption.value;
                this.props.onTrackOptionsChanged(
                  track.uid,
                  track.options
                );
  ...
  getTrackTypeItems(
    ...
    const availableTrackTypes = TRACKS_INFO.filter(
      x =>
        x.datatype
    )
      .filter(
  ...
  getSubmenu() {
    if (
      ...
      if (
        ...
        return this.getTrackTypeItems(
          position,
          bbox,
          track
        );
      }
      return this.getConfigureSeriesMenu(
        position,
        bbox,
        track
      );
