...
class HorizontalHeatmapTrack extends HeatmapTiledPixiTrack {
  ...
  setSpriteProperties(
    ...
    sprite.x = this._refXScale(
      tileX
    );
    sprite.y = this._refYScale(
      tileY
    );
  }
      ...
      if (
        ...
        if (
          ...
          tile
            .renderInfo
            .scaleDomain[1] ===
            this.limitedValueScale.domain()[1]
        ) {
          toUpdate = false;
        }
      }
    ...
    tileProxy.tileDataToPixData(
      ...
      pixData => {
        ...
        if (
          ...
          graphics.pivot.x = this._refXScale(
            0
          );
          graphics.pivot.y = this._refYScale(
            0
          );
          graphics.scale.x =
            -1 /
          ...
          graphics.position.x = this._refXScale(
            0
          );
  ...
  exportSVG() {
    ...
    for (const tile of this.visibleAndFetchedTiles()) {
      const gGraphics = document.createElement(
        'g'
      );
      const graphics =
        tile.graphics;
      const graphicsRotation =
        (graphics.rotation *
          180) /
        Math.PI;
      const transformText = `translate(${
        graphics
          .position
          .x
      },${
