...
vi.describe('Horizontal 1D track tests', () => {
  let hgc = null;
  let api = null;
  let div = null;
  let viewConf;

  vi.describe('Horizontal1DLineTrack with const indicator', () => {
    vi.afterEach(() => {
    ...
    vi.it('check that the const indicators were rendered', async () => {
      ...
      await new Promise((done) => {
        waitForTilesLoaded(hgc, () => {
          vi.expect(trackObj.constIndicator.children.length).to.equal(3);
          done(null);
        });
      });
    });
  ...
  vi.describe('Horizontal1DLineTrack with const indicator', () => {
    ...
    vi.it('check that the const indicators were rendered', async () => {
      ...
      await new Promise((done) => {
        waitForTilesLoaded(hgc, () => {
          // this should be
          vi.expect(trackObj.getDataAtPos(366)).not.to.equal(null);

          // should be beyond the end of the data array because
          // it's past the end of the last tile
          vi.expect(trackObj.getDataAtPos(366)).to.equal(undefined);

          // gene annotations don't currently have a mouseover function
          vi.expect(trackObjGeneAnnotations.getDataAtPos(10)).to.equal(null);
          done(null);
        });
      });
    });
