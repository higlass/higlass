...
import { mountHGComponent, removeHGComponent, waitForJsonComplete } from '../app/scripts/utils';
...
describe('View Config Editor', () => {
  ...
  it('view config should be updated on cmd+s', () => {
    ...
    expect(hgc.instance().state.viewConfig.views[0].tracks.top[0].height).toEqual(60);

    document.dispatchEvent(new KeyboardEvent('keydown', { key: 's', metaKey: true }));
    hgc.update();

    // No changes should now be reflected
    expect(hgc.instance().state.viewConfig.views[0].tracks.top[0].height).toEqual(30);
  });
