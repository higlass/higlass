...
vi.describe('Add track(s)', () => {
  ...
  vi.it('should open the AddTrackDialog', async () => {
    ...
    vi.expect(document.activeElement.className).to.be.eql(
      'tileset-finder-search-box',
    );

    await new Promise((resolve) => waitForJsonComplete(resolve));
  });

  vi.it('should select one plot type and double click', async () => {
  ...
  vi.it('should reopen the AddTrackModal', async () => {
  ...
  vi.it('should select two different plot types', async () => {
    const { tilesetFinder } = hgc.instance().modalRef;
  ...
  vi.it('should add these plot types', async () => {
  ...
  vi.it(
    'should select a few different tracks and check for the plot type selection',
    () => {
      const { tilesetFinder } = hgc.instance().modalRef;
      ...
      vi.expect(ptc.AVAILABLE_TRACK_TYPES.length).to.eql(0);
      ...
      vi.expect(ptc.AVAILABLE_TRACK_TYPES.length).to.eql(3);
      ...
      vi.expect(ptc.state.selectedPlotType.type).to.eql('gene-annotations');
      ...
      vi.expect(hgc.find('.plot-type-item').length).to.eql(0);
    },
  );

  vi.it('should add the selected tracks', () => {
    hgc.instance().modalRef.handleSubmit();
    const viewConf = JSON.parse(hgc.instance().getViewsAsString());

    vi.expect(viewConf.views[0].tracks.top.length).to.eql(6);

    hgc.update();
  });

  vi.it(
    'remove existing tracks & add a new dm6 track: should zoom to dm6 extent',
    async () => {
      ...
      vi.expect(viewConf.views[0].initialXDomain[0]).to.eql(
        tilesetInfo.min_pos[0],
      );
      vi.expect(viewConf.views[0].initialXDomain[1]).to.eql(
  ...
  vi.it(
    'remove existing track & add a new hg19 track: should zoom to hg19 extent',
    async () => {
      // 1. Remove all existing tracks
      const { trackRenderer } = hgc.instance().tiledPlots.aa;
  ...
  vi.it(
    'zoom in, add another hg19 track: the visible domain should not have changed',
    async () => {
      ...
      const newXDomain = [
        ...JSON.parse(hgc.instance().getViewsAsString()).views[0]
          .initialXDomain,
      ];
      ...
      await new Promise((done) => {
        waitForTilesLoaded(hgc.instance(), () => {
          ...
          vi.expect(viewConf.views[0].initialXDomain[0]).to.eql(newXDomain[0]);
          vi.expect(viewConf.views[0].initialXDomain[1]).to.eql(newXDomain[1]);
